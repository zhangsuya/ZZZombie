...
typedef struct BSStackFrameBackTreeEntry{
    int i;
    uintptr_t backtraceBuffer[50];
} BSStackFrameBackTreeEntry;
...
namespace  BackTraceTool{
    ...
    NSString* bs_logBacktraceEntry(const int entryNum,
        ...
        return [NSString stringWithFormat:@"%-30s  0x%08 %s + %lu\n" ,fname, (uintptr_t)address, sname, offset];
    }
    ...
    uintptr_t _bs_backtraceOfThread(thread_t thread) {
        uintptr_t backtraceBuffer[50];
        int i = 0;
        BSStackFrameEntry frame = {0};
        BSStackFrameBackTreeEntry entry ;
        _STRUCT_MCONTEXT machineContext;
        if(!bs_fillThreadStateIntoMachineContext(thread, &machineContext)) {
            entry.i = 0;
            return (uintptr_t)backtraceBuffer;
        }
        ...
        if(instructionAddress == 0) {
            entry.i =0;
//            entry.backtraceBuffer = backtraceBuffer;
            return (uintptr_t)backtraceBuffer;
        }
        ...
        for(; i < 50; i++) {
            entry.backtraceBuffer[i] = frame.return_address;
...
@implementation ZZBackTraceTool
